...
public class FoldButtonBehaivor : MonoBehaviour {
    ...
    async void send_fold_action()
    {
        if (CLEOS.permission_to_make_turn == false)
            return;
        CLEOS.permission_to_make_turn = false;

        Debug.Log("send_fold_action - " + CLEOS.my_table_index.ToString());
        try
        {
            var result = await CLEOS.eos.CreateTransaction(new Transaction()
            {
                Actions = new List<EosSharp.Api.v1.Action>()
                {
                    new EosSharp.Api.v1.Action()
                    {
                        ...
                        Name = "act",
                        Data = new { name = CLEOS.account_name, table_id = CLEOS.table_index, act = new { act_= 3, bet_ = "0.0000 EOS" }, timestamp = CLEOS.last_table_timestamp_abi}
                    }
                }
            });

            transaction_fold_id = result;
            Debug.Log(" send_fold_action result - " + transaction_fold_id);
        }
        catch (EosSharp.Exceptions.ApiErrorException e)
        {
            AlertMessage.GetComponent<UILabel>().text = e.Error.Name + " : " + e.Error.What + e.Error.Details[0].Message;
